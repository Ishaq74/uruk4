...
const ArticleCard: React.FC<{ article: Article, navigateTo: (page: string, id: string) => void }> = ({ article, navigateTo }) => (
  <a href="#" onClick={(e) => { e.preventDefault(); navigateTo('article-detail', article.id); }} className="group block rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 bg-white">
    ...
    <section>
      ...
      <div className="mt-12 grid gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
       ...
       <div className="mt-12 text-center">
        <a href="#" onClick={(e) => { e.preventDefault(); navigateTo('articles'); }} className="px-6 py-3 text-base font-semibold text-white bg-emerald-600 rounded-full hover:bg-emerald-700 transition-all duration-300 shadow-md">
